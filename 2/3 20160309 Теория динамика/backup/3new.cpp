...
// Число двудольных графов на n + m вершинах с k связными областями, 
// причём первые n вершин относятся к доле в которой лежит первая вершина
// (и допускаются вершины степени ноль)
ull bipartite(int n, int m, int k)
{
	if (k == 0)
		return n == 0 && m == 0;
	if (n + m == k)
	...
	// k1 --- число вершин в компоненте связности первой вершины, лежащих в первой доле
	// k2 --- ... во второй
	ull ans = 0;
	for (int k1 = 1; k1 <= n; ++k1)
		for (int k2 = 0; k2 <= m; ++k2)
			if (k1 < n || k2 < m)
				ans += C(n - 1, k1 - 1) * C(m, k2) * bipartite(k1, k2, 1) * bipartite(n - k1, m - k2, k - 1);
	return ans;
}
...
ull bipartite(int n, int k)
{
	...
	if (k == 1)
	{
		...
		for (int k = 1; k <= n; ++k)
			ans += C(n - 1, k - 1) * bipartite(k, n - k, 1);
	}
	else
	{
