...
int main() {
	...
	for (int i = 1; i <= n; ++i) {
		cout << i << " ";
		for (int j = 1; j <= m; ++j) {
			int t = i == xa && j == ya ? 3 : i == xb && j == yb ? 4 : ts[i][j];
			cout << (t == 1 ? '#' : t == 2 ? '*' : t == 3 ? 's' : t == 4 ? 't' : ' ');
		}
		cout << endl;
	}
	//*/
	...
	for (int i = 1; i <= n; ++i) {
		for (int j = 1; j <= m; ++j) {
			bool f1 = used[(i * m + j) * 2];
			bool f2 = used[(i * m + j) * 2 + 1];
			cout << ((f1 && f2) ? 'b' : f1 ? '1' : f2 ? '2' : ' ');
		}
		cout << endl;
	}
	cout << endl;

	for (int i = g.first_edge(25); i != -1; i = g.next_edge(i)) {
		Edge &e = g.edges[i];
		printf("%d->%d: %d/%d\n", e.u, e.v, e.f, e.c);
	}
	 //*/
